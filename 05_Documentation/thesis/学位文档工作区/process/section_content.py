# -*- coding: utf-8 -*-
"""开题报告 SECTION1/2/3 正文（不含参考文献列表；列表格式见 bibliography_format.py / BIBLIOGRAPHY.yaml）
...
半导体产业是国民经济与国家安全的重要支柱，其制造链条可概括为前道晶圆制造与后道封装测试两大环节。后道封装测试OSAT承担贴带包装、键合研磨、晶圆切割、粘片、焊线及塑封等工序，是连接芯片设计与终端应用的关键枢纽。全球半导体分工深化以来，OSAT企业普遍服务多家无晶圆设计客户，在MTO按单生产模式下同时处理多品种、小批量、短交期订单，排程决策直接影响制造周期、准交率、在制品WIP水平及设备利用率。Chien等（2011）指出，半导体制造建模需在产能约束、工艺路线柔性、随机扰动与信息不完全之间取得平衡；对于封装后端而言，上述矛盾往往体现为物流驱动型排程问题，而非单纯的前道工艺优化问题。
...
从业务视角看，OSAT企业订单在ReleaseDate、DueDate、Material类型、OrderQty等方面高度异质。Lin和Chen（2015）在对真实封装组装设施的混合流水排程研究中指出，订单常被拆分为若干子作业并行加工后再合并，且同一订单的子作业在各阶段须使用相容的机台类型，以满足质量追溯与产品—机台资格约束。这一拆分—并行—合并行为显著增加了释放控制、批次规模与派工决策的耦合复杂度。Fang等（2023）在半导体制造机器调度综述中强调，后端组装与前道晶圆厂相比更突出物流、交期与流动时间Flow Time管理问题；短制造周期与稳健准交能力直接关联库存风险、销售损失与客户满意度。Sang-Jin Lee 与 Tae-Eog Lee（2008，2011）针对MCP多芯片封装组装线的研究亦表明，重入工序、异构并行机与换型时间对产能分配策略具有决定性影响，与本研究DA/WB结构高度相关。
...
第一，丰富封装后端排程建模范式。在统一Simio模型中同时刻画六工序流转、Order—MLot—Magazine—SubLot批次链拆分与合并、Worker搬运与DA/WB换型Setup，有助于弥补简化流水模型对OSAT场景刻画不足的问题，回应Chien等（2011）与Chen-Ritzo等（2011）关于仿真需贴近工业逻辑方能支持运营决策的论断。

第二，拓展仿真优化在后端组装的应用边界。以MakeSpan与延期惩罚构建加权多目标Response，并借助Scenario Generator开展二维因子网格实验，为Mönch等（2011）与Lin和Chen（2015）所总结的仿真评估与参数搜索范式提供可复现案例；与Chen等（2013）GA/PSO+OCBA、Chiu等（2023，2026）智能优化研究形成方法对照，明确DES Experiment在工业级复杂约束下的基准作用。
...
半导体制造调度研究长期以前道晶圆厂为主，但后端组装同样面临多品种、并行机台、换型与交期约束等难题。Fang等（2023）系统梳理了半导体制造机器调度问题及精确算法、启发式与仿真优化等求解方法，指出后端场景在订单拆分、异构并行机与交期管理方面具有鲜明特征。刘琼等（2022）从中文文献角度综述了半导体制造调度问题，为国内研究提供了系统入口。Chien等（2011）从全球产业链视角总结了半导体制造建模分析的主要挑战，强调脱敏真实数据与仿真验证对管理决策支持的重要性。

在封装组装线领域，Sang-Jin Lee 与 Tae-Eog Lee（2008，2011）针对MCP组装线研究了重入工序、异构并行机与换型下的产能分配与派工策略，是后端排程的代表性工作。Lin和Chen（2015）基于真实OSAT数据提出混合流水仿真优化方法，以遗传算法结合最优计算预算分配OCBA求解产线—机台类型指派，并开展需求、产品结构与批次大小情景分析，与本研究企业脱敏数据及仿真优化路线高度契合。Chen等（2013）将粒子群优化PSO与OCBA用于后段组装动态并行机调度，验证了元启发式与仿真联动的有效性。赵子夜等（2025）针对封装测试柔性流水车间建立含拖期惩罚的多目标模型；轩华等（2020）研究考虑运输的柔性流水车间调度，为混合流水扩展提供参考。

从派工规则视角看，Panwalkar和Iskander（1977）的经典综述表明，EDD、SPT、CR等规则在不同目标下表现各异；在交期导向场景中，EDD常作为基准规则，但其效果依赖释放策略与队列形成机制。文献与行业经验均表明，Source释放顺序与设备队列派工规则若未与EDD协同，Penalty触发情景下可能出现高额延期惩罚，说明后端排程需同时优化入流顺序与队列排序。Sang-Jin Lee 与 Tae-Eog Lee（2011）的产能分配策略亦提示，仅优化局部队列不足以解决全局交期问题，需与订单释放、批次拆分协同。本研究拟通过Source层EDD实现与Orders表行序对照实验，检验上述机制在本场景下的作用边界。

近年来，智能优化方法加速向后端延伸。Chiu等（2023）研究半导体组装订单分配与灵活拆分规则的进化仿真优化；Chiu等（2026）将深度强化学习与仿真优化集成于后端组装调度。Yedidsion等（2022）与Zhou等（2020）在晶圆厂/智能制造场景下展示了深度强化学习对动态调度的潜力。Hoppe等（2025）提出结构化强化学习SRL，通过嵌入组合优化层处理大规模组合动作空间；Luttmann和Xie（2026）的多动作自改进方法MACSIM在柔性流水/作业车间调度基准上展示了神经组合优化进展。上述研究丰富了方法谱系，但面向六工序一体化、含Worker搬运、换型Setup与Order—MLot—Magazine—SubLot批次链的Simio Experiment框架仍相对少见，且部分智能方法对数据规模与训练成本要求较高，尚难直接替代工业DES上的可解释参数扫描。

3.2 批次拆分、批处理与lot streaming

批次与子批次决策深刻影响WIP水平、流动时间与设备利用率。Potts等（1989）奠定了多阶段调度中lot streaming分解的理论基础；Uzsoy等（1992）给出同构子批的单机调度算法。Klemmt等（2011）比较混合整数规划与离散事件仿真，提出仿真与数学规划结合的时间窗分解方法，是半导体批处理调度的重要参考。Mönch等（2003）通过仿真评估批处理启发式对延期指标的影响；Mönch等（2005）研究不兼容产品族并行批处理机的启发式调度；Habenicht等（2002）将有限容量束搜索算法集成于晶圆厂生产调度。Stubbe和Rose（2011）分析小批量与批处理设备替换的仿真绩效。王卓君等（2023）研究晶圆批处理设备的组批与指派。
...
现有文献多聚焦前道炉管批处理或一般lot streaming，对Order—MLot—Magazine—SubLot批次链、换型Setup、搬运与多目标Penalty的组合研究相对有限。本研究将在Klemmt等（2011）与Stubbe和Rose（2011）的仿真优化思路上，结合Simio Combiner/Separator逻辑，形成可复现实验网格，填补封装后端批次参数扫描的实证空白。

3.3 离散事件仿真与仿真优化

Law（2015）、Banks等（2010）系统阐述了DES建模、验证、输出分析与实验设计方法论，是管理科学与工程领域开展仿真研究的经典教材。Chen-Ritzo等（2011）总结了半导体制造中仿真支持运营决策的工业实践经验，指出仿真模型需与MES/计划系统数据口径一致。Mönch等（2011）综述了半导体制造调度问题与求解技术，强调仿真优化在随机环境下的必要性。商业DES平台Simio支持Experiment模块与Scenario Generator，便于对批次因子、释放规则等离散或连续决策变量进行网格扫描，并为后续接入OptQuest等仿真优化器预留接口。本研究拟采用Scenario Generator全因子扫描MLotFactor×SubLotFactor，并开展释放顺序对照及EDD/FIFO对比，属于仿真优化中的序贯敏感性分析策略。

仿真优化一般包括三个环节：以仿真模型作为仿真性能评估器；以优化或搜索策略在决策空间上引导采样；以OCBA、Common Random Numbers等统计方法处理仿真噪声。Lin和Chen（2015）与Chen等（2013）代表了元启发式与OCBA路线；本研究开题至中期阶段以Scenario Generator穷举/网格为主，优势在于对工业参数的直接可解释性与较低实现门槛。Mönch等（2011）亦指出，在复杂系统中，先用仿真摸清参数敏感性再嵌入优化器，是常见且稳健的技术路线。
...
MakeSpan统计口径方面，Experiment拟并行采用Mold工序截面时刻Mold_T_End与Sink整线LastDepartureTime两种记录方式，并以后者作为全局制造周期主口径。两种口径在末道MOLD工序截面与Sink整线完工程度之间可能产生差异。Chen-Ritzo等（2011）强调KPI定义与仿真事件记录点必须一致；本研究将在实验中并行记录两种MakeSpan，并在论文中报告口径敏感性，避免结论对外部读者产生歧义。

3.5 文献述评与研究缺口

综合上述研究，可以得到以下缺口。第一，面向封装后端六工序一体化、含Worker搬运、DA/WB换型Setup及Order—MLot—Magazine—SubLot批次链的DES模型与脱敏真实订单数据相结合的实证研究仍不足。第二，在MakeSpan与延期惩罚双目标下，批次因子与EDD释放/派工策略的联合优化及对比验证有待系统化；文献与行业经验提示，仅调整订单表行序可能不足以实现交期驱动释放，本研究拟通过Source层EDD与表序对照实验加以检验。第三，负荷校准、ReleaseDate/DueDate基准化与Penalty触发机制尚未在统一实验协议下充分揭示。本研究将在上述缺口上开展可复现的Simio Experiment研究，并为后续OptQuest或智能优化对照预留接口。

3.6 本研究与现有研究的定位

与Lin和Chen（2015）聚焦产线—机台类型指派、GA与OCBA相比，本研究将决策重点放在批次缩放因子与释放/派工规则，以Simio Experiment Scenario Generator为平台，强调MakeSpan与Penalty加权多目标在固定工艺路线下的参数敏感性，而非重新求解指派组合爆炸。与Chiu等（2023，2026）的智能优化路线相比，本研究优先保证工业级DES模型的可解释性与可复现性，使实验结论可直接对接计划员可操作的MLotFactor、SubLotFactor与EDD规则。与Klemmt等（2011）、Stubbe和Rose（2011）的批处理仿真研究相比，本研究突出封装后端Order—MLot—Magazine—SubLot批次链及Worker搬运与换型Setup约束，而非前道炉管批处理。与Hoppe等（2025）、Luttmann和Xie（2026）等组合优化/神经方法相比，本研究提供基于真实脱敏订单的基准场景，可作为未来智能调度算法对照的数字孪生底座。
...
第四，情景分析与敏感性分析。参照Stubbe和Rose（2011）、Sang-Jin Lee 与 Tae-Eog Lee（2011）的做法，在不同负荷水平、因子水平下比较系统绩效，解释Penalty触发条件与瓶颈迁移现象。
...
第一，在单一Simio模型中集成六工序、Worker搬运、DA/WB换型Setup及Order—MLot—Magazine—SubLot批次链，贴近OSAT后端实际逻辑，克服简化流水模型对封装场景刻画不足的问题。
