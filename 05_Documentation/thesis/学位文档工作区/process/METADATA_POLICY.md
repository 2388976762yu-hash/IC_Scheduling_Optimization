...
**[`2120253828-喻炫琪-研究生开题报告.doc`](../../2120253828-喻炫琪-研究生开题报告.doc)**（根目录 **提交件**）

**学院空白模板 / 填写说明**（对照版式，脚本不覆盖）→ [`../../参考资料/00_README.md`](../../参考资料/00_README.md)
...
## 中期报告 / 考核表（`fill_midterm.py`）

**默认：只在本地提交 `.doc` 上增量修改，不从空白模板覆盖。**

| 规则 | 说明 |
|------|------|
| 真源 | `开题报告/2120253828-…中期报告.doc` 与 `…中期考核表.doc`（**你在 WPS 里保存的版本**） |
| 空白模板 | 仅 `开题报告/参考资料/学院原版空白模板/`，**对照用** |
| 默认行为 | 只填**空单元格**；已有文字、勾选、摘要**一律跳过** |
| 禁止默认 | **不得** `shutil.copy` 空白模板覆盖提交件 |
| 学院填写区 | 考核表 Table2 R6/R7（思想品德、课程学习）、Table3 **脚本 never 写入** |
| 显式重置 | 仅当你本人要求时：`python fill_midterm.py --restore-from-blank`（会丢失本地修改） |
| 强制覆盖摘要 | `--abstract-only --force`（**仅**改 R5 摘要，不碰勾选项） |
| 勾选行 | 默认**不写入**；仅 `--fill-checkboxes` 且单元格内尚无 √ 时才打勾 |
| 重写中期报告正文 | `--rewrite-body`（危险，默认不启用） |

AI 协作者：**未经用户明确要求，不要运行** `fill_midterm.py`；若运行，**不得**带 `--restore-from-blank` / `--rewrite-body`。
