...
从业务视角看，OSAT企业订单在ReleaseDate、DueDate、Material类型、OrderQty等方面高度异质。Lin和Chen（2015）在对真实封装组装设施的混合流水排程研究中指出，订单常被拆分为若干子作业并行加工后再合并，且同一订单的子作业在各阶段须使用相容的机台类型，以满足质量追溯与产品—机台资格约束。这一“拆分—并行—合并”行为显著增加了释放控制、批次规模与派工决策的耦合复杂度。Wu等（2023）在半导体制造机器调度综述中强调，后端组装与前道晶圆厂相比更突出物流、交期与流动时间（Flow Time）管理问题；短制造周期与稳健准交能力直接关联库存风险、销售损失与客户满意度。Lee和Lee（2008，2011）针对多芯片封装（MCP）组装线的研究亦表明，重入工序、异构并行机与换型时间对产能分配策略具有决定性影响，与本研究DA/WB结构高度相关。

在实际排程实践中，计划人员通常同时关注两类相互制约的目标：一是系统层面制造周期（MakeSpan）或流动时间最小化，反映产线吞吐效率；二是订单层面准交与延期惩罚最小化，反映交期承诺履行能力。Pinedo（2016）系统论述了制造周期、拖期与误期等经典目标函数；在多目标无法同时最优时，加权标量化是管理科学与工程领域常用的决策支持方式。本研究采用Objective=0.7×MakeSpan+0.3×Penalty，与项目组Experiment实现及决策记录（D-004）保持一致，其中Penalty在出现订单延期时取大罚值、全部准时则为零，以突出准交约束的硬边界特征。

1.3 本研究数据来源与研究场景

本研究依托导师科研项目，基于典型存储类半导体封装企业提供的已脱敏行业订单、物料与设备配置数据开展研究。数据经脱敏处理后保留订单数量、ReleaseDate/DueDate、Material类型、各工序单位产能（UPH）及机台规模等字段，不含可识别客户或合作方信息。基准场景包含41张订单，代表“一周”生产负荷；设备规模合计459台（D/S 31、DA 112、WB 284、Mold 7、B/G 17、Taping 8），加工时间按CurrentQty/UPH计算，DA/WB换型含搬运3分钟与设置10分钟（MaterialID/Group变化触发）。

在模型层面，采用Simio离散事件仿真（Discrete Event Simulation，DES）构建六工序一体化模型，刻画三级批次结构（制造批次MLot→杂志批次Magazine→子批次SubLot）、Worker搬运、换型及Source订单释放逻辑。Law（2015）与Banks等（2010）所阐述的DES建模—验证—实验设计流程，为本研究提供了方法规范。研究决策变量包括批次缩放因子MLotFactor、SubLotFactor（通过Model Properties接入Scenario Generator），以及基于最早交期规则（Earliest Due Date，EDD）的订单释放与派工策略。前期EXP-001至EXP-003实验表明：Experiment绩效链路可正确计算MakeSpan、Penalty与Objective；在SPAN基准数据下，26场景因子网格扫描得到Objective最优50.61（MLot=1.0，SubLot=0.75），部分因子组合Penalty高达760；仅调整Orders表DueDate行序不足以替代Source层EDD逻辑。上述结果为开题阶段的可行性提供了实证支撑。
...
第三，揭示效率—准交权衡结构。通过对比FIFO、EDD等经典派工规则（Panwalkar和Iskander，1977）及批次因子敏感性，有助于识别MakeSpan与Penalty的Pareto权衡区间，补充Pinedo（2016）调度理论在含批次拆分与交期惩罚场景下的实证讨论。Framinan等（2004）与Ruiz等（2006）关于流水/混合流水制造周期启发式的研究，为本研究解析因子—规则交互效应提供了理论参照。
...
对于企业生产计划部门而言，本研究输出的并非一次性最优解，而是一组可解释的参数—绩效映射关系：在给定脱敏订单结构与设备规模下，何种批次缩放区间更可能同时控制MakeSpan与Penalty；在何种负荷水平下瓶颈由“喂料不足”转向“DA/WB约束”；EDD释放相对于FIFO究竟改善多少Penalty。此类“what-if”洞察正是Chen-Ritzo等（2011）所强调的仿真决策支持价值。对于学院管理科学与工程训练而言，本研究贯穿数据治理、随机系统建模、实验设计与多目标决策，符合系统建模与仿真研究方向的人才培养目标。
...
在封装组装线领域，Lee和Lee（2008，2011）针对MCP组装线研究了重入工序、异构并行机与换型下的产能分配与派工策略，是后端排程的代表性工作。Lin和Chen（2015）基于真实OSAT数据提出混合流水仿真优化方法，以遗传算法结合最优计算预算分配（OCBA）求解产线—机台类型指派，并开展需求、产品结构与批次大小情景分析，与本研究“真实数据+SO”路线高度契合。Chen等（2013）将粒子群优化（PSO）与OCBA用于后段组装动态并行机调度，验证了元启发式与仿真联动的有效性。赵子夜等（2025）针对封装测试柔性流水车间建立含拖期惩罚的多目标模型；轩华等（2020）研究考虑运输的柔性流水车间调度，为混合流水扩展提供参考。

从派工规则视角看，Panwalkar和Iskander（1977）的经典综述表明，EDD、SPT、CR等规则在不同目标下表现各异；在交期导向场景中，EDD常作为基准规则，但其效果依赖释放策略与队列形成机制。本研究前期实验显示，在Penalty触发情景下，Source释放顺序与Server Selection Rule尚未与EDD一致，导致部分因子组合出现高额延期惩罚，说明后端排程需同时优化“入流顺序”与“队列排序”。Lee和Lee（2011）的产能分配策略亦提示，仅优化局部队列不足以解决全局交期问题，需与订单释放、批次拆分协同。
...
在封装后端，批次结构更具层次性：制造批次（MLot）→杂志批次（Magazine）→子批次（SubLot）。Lin和Chen（2015）指出，Magazine是跨工序搬运与追溯的关键单元，拆分粒度直接影响作业在DA/WB等热点工序的并行度与等待时间。本研究通过MLotFactor、SubLotFactor对Separator逻辑进行缩放，在不改动底层BOM的前提下模拟“批次放大/缩小”政策，属于参数化敏感性分析而非重新设计Lot结构。EXP-002结果显示，SubLotFactor=0.75在MLot=1.0附近取得较优Objective，说明适度减小子批可能缩短流动时间；但当因子过大（1.25×1.25）时Penalty与MakeSpan同时恶化，体现批次规模与交期约束的非线性耦合。

现有文献多聚焦前道炉管批处理或一般lot streaming，对“三级批次+换型+搬运+多目标Penalty”的组合研究相对有限。本研究将在Klemmt等（2011）与Stubbe和Rose（2011）的仿真优化思路上，结合项目组Model3.0已实现的Combiner/Separator逻辑，形成可复现实验网格，填补封装后端批次参数扫描的实证空白。

3.3 离散事件仿真与仿真优化

Law（2015）、Banks等（2010）系统阐述了DES建模、验证、输出分析与实验设计方法论，是管理科学与工程领域开展仿真研究的经典教材。Chen-Ritzo等（2011）总结了半导体制造中仿真支持运营决策的工业实践经验，指出仿真模型需与MES/计划系统数据口径一致。Fowler等（2008）综述了基于仿真的调度研究进展，强调仿真优化在随机环境下的必要性。商业DES平台（如Simio）支持Experiment模块与Scenario Generator，便于对批次因子、释放规则等离散/连续决策变量进行网格扫描，并为后续接入OptQuest等仿真优化器预留接口。本研究采用Scenario Generator全因子扫描MLotFactor×SubLotFactor，对标项目组EXP-002/003实验协议，属于SO中的序贯敏感性分析策略。

仿真优化一般包括：（1）仿真模型作为性能评估oracle；（2）优化/search策略在决策空间上引导采样；（3）统计方法处理仿真噪声（如OCBA、Common Random Numbers等）。Lin和Chen（2015）与Chen等（2013）代表了元启发式+OCBA路线；本研究当前阶段以Scenario Generator穷举/网格为主，优势在于对工业参数的直接可解释性与较低实现门槛，符合开题到中期“先建立可复现基准”的研究节奏。Fowler等（2008）亦指出，在复杂系统中，先用仿真摸清参数敏感性再嵌入优化器，是常见且稳健的技术路线。
...
在多目标处理策略上，常见方法包括Pareto前沿求解、分层优化与加权标量化。本研究采用Objective=0.7×MakeSpan+0.3×Penalty，属于加权标量化，便于在Simio Experiment中作为单一Primary Response排序Scenario，与项目组已跑通的EXP-001/002/003实现一致。Penalty采用“延期则大罚、全准时为零”的阈值式设计，强调准交硬约束，与部分文献中连续拖期积分目标不同；该设计更贴近计划部门对“是否延期”的离散管理口径，但需在论文中明确Penalty标度对权重解读的影响。

MakeSpan统计口径方面，DECISIONS D-002记录当前Experiment暂用Mold_T_End，而导师会议建议采用Sink整线LastDepartureTime。两种口径在数值上可能接近，但在订单后段（B/G、Taping）占用显著时将产生差异。Chen-Ritzo等（2011）强调KPI定义与仿真事件记录点必须一致；本研究将在后续实验中并行记录两种MakeSpan，并在论文中报告口径敏感性，避免结论对外部读者产生歧义。

3.5 文献述评与研究缺口

综合上述研究，可以得到以下缺口：（1）面向封装后端六工序一体化、含Worker搬运、DA/WB换型及三级批次结构的DES模型与脱敏真实订单数据相结合的实证研究仍不足；（2）在MakeSpan与延期惩罚双目标下，批次因子与EDD释放/派工策略的联合优化及对比验证有待系统化——EXP-003已表明仅调整Excel行序不足以实现交期驱动释放；（3）负荷校准、ReleaseDate/DueDate基准化与Penalty触发机制尚未在统一实验协议下充分揭示。本研究将在上述缺口上开展可复现的Simio Experiment研究，并为后续OptQuest或智能优化对照预留接口。

3.6 本研究与现有研究的定位

与Lin和Chen（2015）聚焦“产线—机台类型指派+GA+OCBA”相比，本研究将决策重点放在批次缩放因子与释放/派工规则，以Simio Experiment Scenario Generator为平台，强调加权多目标（MakeSpan+Penalty）在固定工艺路线下的参数敏感性，而非重新求解指派组合爆炸。与Chiu等（2023，2026）的智能优化路线相比，本研究优先保证工业级DES模型的可解释性与可复现性，使实验结论可直接对接计划员可操作的参数（MLotFactor/SubLotFactor、EDD规则）。与Klemmt等（2011）、Stubbe和Rose（2011）的批处理仿真研究相比，本研究突出封装后端三级批次结构及Worker/换型约束，而非前道炉管批处理。与Hoppe等（2025）、Luttmann和Xie（2026）等组合优化/神经方法相比，本研究提供基于真实脱敏订单的基准场景，可作为未来智能调度算法对照的“数字孪生”底座。

从研究方法论链条看，本研究遵循“数据基准化→DES建模→KPI/Experiment→因子与规则扫描→负荷校准→论文凝练”的递进逻辑，与Law（2015）所强调的仿真研究规范一致；在优化搜索层面，当前阶段以全因子网格为主（EXP-002/003），后续可逐步引入OptQuest或借鉴Chen等（2013）的PSO+OCBA进行连续/离散混合寻优。在绩效测度层面，将同步推进MakeSpan口径统一（Mold截面 vs Sink整线）与Penalty触发机制分析，避免KPI混用导致结论不可比（Chen-Ritzo等，2011）。
