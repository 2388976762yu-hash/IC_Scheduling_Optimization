...
近年来，智能优化方法加速向后端延伸。Chiu等（2023）研究半导体组装订单分配与灵活拆分规则的进化仿真优化；Chiu等（2026）将深度强化学习与仿真优化集成于后端组装调度。Yedidsion等（2021）与Zhou等（2020）在晶圆厂/智能制造场景下展示了深度强化学习对动态调度的潜力。Hoppe等（2025）提出结构化强化学习SRL，通过嵌入组合优化层处理大规模组合动作空间；Luttmann和Xie（2026）的多动作自改进方法MACSIM在柔性流水/作业车间调度基准上展示了神经组合优化进展。上述研究丰富了方法谱系，但面向六工序一体化、含Worker搬运、换型Setup与Order—MLot—Magazine—SubLot批次链的Simio Experiment框架仍相对少见，且部分智能方法对数据规模与训练成本要求较高，尚难直接替代工业DES上的可解释参数扫描。
