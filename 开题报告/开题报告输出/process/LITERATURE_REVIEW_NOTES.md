...
- **Lee and Lee (2011)**：MCP 组装线，DA/WB 重入、并行异构机台、换型显著
- **Lee and Lee (2008)**：WSC 会议版，产能分配策略降低 setup 同时维持 OTD
...
**述评要点**：后端组装文献聚焦 MCP/HFSP，但较少同时考虑三级 lot 拆分 + 交期惩罚 + 商业 DES 平台 Experiment。
...
**述评要点**：批次拆分对 WIP/流动时间影响明确，但封装后端三级拆分（MLot/Mag/SubLot）与 Experiment 因子扫参结合研究较少。
...
1. 面向 **封装后端六工序** 的一体化 DES 模型（含 Worker、换型、三级批次）与 **真实脱敏订单** 的结合研究不足。
